...
class Solution {
    public int minRefuelStops(int target, int startFuel, int[][] stations) {
        ...
        Queue<int> pq = new PriorityQueue<>((a,b)->b-a);
        
        int numOfStops = 0, current = 0;
        
        for (;current < stations.length; current++) {
            if(startFuel == stations[current][0]) {
                startFuel += stations[current][1];
                numOfStop += 1;
            } else if(start > stations[current][0]) {
